...
async function doLogin() {
  ...
  try {
    ...
    if (errorCode === 'auth/user-not-found') {
      errorEl.innerHTML = '<strong>Usuario no encontrado</strong><br>Este email no está registrado en el sistema. Contactá al administrador para que te cree la cuenta, o verificá que estés usando el email correcto.';
    } else if (errorCode === 'auth/wrong-password') {
      errorEl.innerHTML = '<strong>Contraseña incorrecta</strong><br>Si no recordás tu contraseña, usá el link "Olvidé mi contraseña" debajo.';
    } else if (errorCode === 'auth/invalid-email') {
      errorEl.innerHTML = '<strong>Email inválido</strong><br>El formato del email no es correcto. Ejemplo válido: usuario@gmail.com';
    } else if (errorCode === 'auth/user-disabled') {
      errorEl.innerHTML = '<strong>Cuenta deshabilitada</strong><br>Tu cuenta fue desactivada. Contactá al administrador.';
    } else if (errorCode === 'auth/too-many-requests') {
      errorEl.innerHTML = '<strong>Demasiados intentos</strong><br>Por seguridad, bloqueamos temporalmente el acceso. Probá de nuevo en unos minutos.';
    } else if (errorCode === 'auth/network-request-failed') {
      errorEl.innerHTML = '<strong>Error de conexión</strong><br>No se pudo conectar con el servidor. Verificá tu conexión a internet.';
    } else if (errorMessage.includes('no registrado en el sistema')) {
      errorEl.innerHTML = '<strong>Usuario incompleto</strong><br>La cuenta existe en autenticación pero falta en la base de usuarios. Contactá al administrador.';
    } else {
      errorEl.innerHTML = `<strong>Error de acceso</strong><br>${errorMessage || 'Usuario o contraseña incorrectos'}`;
    }
    errorEl.classList.add('active');
  } finally {
    setLoading(false);
...
async function recuperarPassword() {
  ...
  if (!user || !user.includes('@')) {
    errorEl.innerHTML = '<strong>Ingresá tu email</strong><br>Escribí tu dirección de email arriba y luego hacé clic en "Olvidé mi contraseña".';
    errorEl.classList.add('active');
  ...
  if (!STATE.auth) {
    errorEl.innerHTML = '<strong>Error</strong><br>Firebase no está configurado.';
    errorEl.classList.add('active');
    return;
  }
  ...
  try {
    ...
    errorEl.innerHTML = `<strong>✅ Email enviado</strong><br>Revisá tu bandeja de entrada (${user}). Si no lo encontrás, mirá en spam.`;
    errorEl.classList.add('active');
  } catch (e) {
    console.error('[RESET ERROR]', e);
    const code = e.code || '';
    if (code === 'auth/user-not-found') {
      errorEl.innerHTML = '<strong>Usuario no encontrado</strong><br>No existe una cuenta con ese email.';
    } else if (code === 'auth/invalid-email') {
      errorEl.innerHTML = '<strong>Email inválido</strong><br>El formato del email no es correcto.';
    } else {
      errorEl.innerHTML = `<strong>Error</strong><br>No se pudo enviar el email: ${e.message}`;
    }
    errorEl.classList.add('active');
  } finally {
    setLoading(false);
...
Respondé en español, con formato Markdown, incluyendo tablas si es necesario.`
    ...
    if (tbody) {
      ...
      tbody.innerHTML = servicios.map(svc => {
        ...
        return `<tr><td>${svc}</td><td style="text-align:center;">${fmtMoney(saldo)}</td><td style="text-align:center;">${fmtMoney(consumoProm)}</td><td style="text-align:center;">${fmtMoney(proy)}</td><td style="text-align:center;">${fmtMoney(saldoProy)}</td><td style="text-align:center;"><span class="estado-badge estado-${estado}">${estadoLabel}</span></td></tr>`;
      }).join('');
    }
