...
async function doLogin() {
  ...
  try {
    ...
    if (!userDoc.exists) {
      throw new Error('Usuario autenticado pero no registrado en el sistema de usuarios');
    }
    ...
    // Fallback a usuario de emergencia
    if (typeof USUARIO_EMERGENCIA !== 'undefined' && user === USUARIO_EMERGENCIA.username.toLowerCase() && pass === USUARIO_EMERGENCIA.password) {
    ...
    const msgs = {
      'auth/user-not-found': 'Usuario o contraseña incorrectos. Verificá tus datos o contactá al administrador.',
      'auth/wrong-password': 'Usuario o contraseña incorrectos. Si no recordás tu contraseña, usá "Olvidé mi contraseña".',
      'auth/invalid-credential': 'Usuario o contraseña incorrectos. Verificá tus datos.',
      'auth/invalid-email': 'El email ingresado no tiene un formato válido. Ejemplo: usuario@gmail.com',
    ...
    // Si no es un error conocido de Firebase, mostrar mensaje genérico
    // NUNCA mostrar e.message al usuario porque puede contener HTML/JSON crudo
    if (!msg) {
      msg = 'Usuario o contraseña incorrectos. Verificá tus datos e intentá de nuevo.';
    }

    // Usar textContent para evitar que cualquier HTML/JS se renderice
...
async function recuperarPassword() {
  ...
  if (!user || !user.includes('@')) {
    errorEl.textContent = 'Ingresá tu email arriba y luego hacé clic en "Olvidé mi contraseña".';
    errorEl.classList.add('active');
  ...
  if (!STATE.auth) {
    errorEl.textContent = 'Firebase no está configurado. Contactá al administrador.';
    errorEl.classList.add('active');
    return;
  }
  ...
  setLoading(true);
  try {
    await STATE.auth.sendPasswordResetEmail(user);
    errorEl.textContent = '✅ Email de recuperación enviado. Revisá tu bandeja de entrada y la carpeta de spam.';
    errorEl.classList.add('active');
  } catch (e) {
    ...
    const resetMsgs = {
      ...
      'auth/invalid-email': 'El formato del email no es correcto.',
    };
    errorEl.textContent = resetMsgs[code] || 'No se pudo enviar el email. Intentá más tarde.';
    errorEl.classList.add('active');
  } finally {
    setLoading(false);
  }
...
function cerrarSesion() {
  ...
  STATE.data = {
    ...
    gcSaldos: [], gcCondicion: [], consumoDiario: [], simo: [], lubricantes: [], ypfTotalesDTRA: {}
  };
  STATE.files = {};
  destroyCharts();
  updateUI();
...
function editarUsuario(idx) {
  ...
  const nuevoRol = prompt('Rol (admin/operador/user):', u.rol);
  if (nuevoRol === null) return;

  const actualizado = {
    nombre: nuevoNombre.trim() || u.nombre,
    dni: nuevoDNI.trim() || u.dni,
    rol: ['admin', 'operador', 'user'].includes(nuevoRol) ? nuevoRol : u.rol
  };
...
async function guardarParteDiario() {
  ...
  if (!SESSION || (SESSION.rol !== 'admin' && SESSION.rol !== 'operador')) {
    showAlert('error', 'Solo administradores y operadores pueden guardar partes');
    return;
  }
  ...
  if (!hasData) {
    showAlert('error', 'No hay datos cargados. Arrastrá los 2 archivos Excel primero.');
    return;
  }
  ...
  // Validación de hora de corte: lo que suman los contratos YPF en Ruta debe
  // coincidir con el SALDO TOTAL del bloque YPF de SALDOS DTRA.
  const sumaYpf = STATE.data.ypfTerrestre.reduce((s, r) => s + (parseFloat(r.saldo_actual) || 0), 0)
    + STATE.data.ypfSuperficie.reduce((s, r) => s + (parseFloat(r.saldo_actual) || 0), 0);
  const dtraTotal = (STATE.data.ypfTotalesDTRA || {}).saldo_total || 0;
  if (dtraTotal > 0 && Math.abs(sumaYpf - dtraTotal) > 1) {
    showAlert('error', `⚠️ Hora de corte desactualizada: YPF en Ruta suma ${fmtMoney(sumaYpf)} pero SALDOS DTRA dice ${fmtMoney(dtraTotal)} (diferencia ${fmtMoney(sumaYpf - dtraTotal)}). Revisá las planillas antes de guardar.`);
  }
...
async function listarPartesGuardados() {
  ...
  try {
    ...
    // ---- Carpetas lógicas por mes: no mostrar 30+ partes en fila ----
    const partesArr = [];
    snapshot.forEach(doc => partesArr.push({ fecha: doc.id, data: doc.data() }));

    const porMes = {};
    partesArr.forEach(p => {
      const mes = p.fecha.slice(0, 7); // YYYY-MM
      (porMes[mes] = porMes[mes] || []).push(p);
    });
    const meses = Object.keys(porMes).sort().reverse(); // mes más reciente primero
    ...
    container.innerHTML = meses.map((mes, idx) => {
      ...
      const body = items.map(p => {
        const res = p.data.resumen || {};
        return `
        <div class="repo-item" onclick="cargarParteDelDia('${p.fecha}')" data-fecha="${p.fecha}">
          <div>
            <div class="repo-item-fecha">📅 ${p.fecha}</div>
          ...
          <div class="repo-item-actions">
            <button class="repo-item-btn view" onclick="event.stopPropagation();cargarParteDelDia('${p.fecha}')" title="Ver">👁</button>
            ${SESSION && SESSION.rol === 'admin' ? `<button class="repo-item-btn" onclick="event.stopPropagation();eliminarParte('${p.fecha}')" title="Eliminar">🗑</button>` : ''}
          </div>
        </div>`;
      }).join('');

      return `
      <div class="repo-mes ${idx === 0 ? 'open' : ''}">
        <div class="repo-mes-header" onclick="this.parentElement.classList.toggle('open')">
          <span class="repo-mes-caret">▶</span>
          <span>📁 ${nombreMes(mes)}</span>
          <span class="repo-mes-count">${items.length} parte(s)</span>
        </div>
        <div class="repo-mes-body">${body}</div>
      </div>`;
    }).join('');

    showAlert('success', `${partesArr.length} partes cargados desde Firebase (agrupados por mes)`);
  } catch (e) {
  ...
  try {
    ...
    if (tbody) {
      tbody.innerHTML = indicadores.map(ind => {
        ...
        const sign = diff > 0 ? '+' : '';
        return `<tr><td style="text-align:center;">${ind.nombre}</td><td style="text-align:center;font-family:monospace;">${fmtMoney(ind.a)}</td><td style="text-align:center;font-family:monospace;">${fmtMoney(ind.b)}</td><td style="text-align:center;font-family:monospace;" class="comp-delta ${cls}">${sign}${fmtMoney(diff)}</td><td style="text-align:center;" class="comp-delta ${cls}">${sign}${pct}%</td></tr>`;
      }).join('');
    }
...
async function generarPrevision() {
  ...
  try {
    const snapshot = await STATE.db.collection('partes').orderBy('meta.fecha', 'desc').get();
    const partes = {};
    snapshot.forEach(doc => { partes[doc.id] = doc.data(); });
    // partes ya definido arriba desde Firebase
...
Respondé en español, con formato Markdown, incluyendo tablas si es necesario.`
    ...
    // ===== Proyección por servicio con consumo histórico REAL =====
    // El consumo diario de cada servicio = MEDIANA de las bajas de saldo entre
    // partes consecutivos (ignora acreditaciones/OP que hacen SUBIR el saldo).
    // Es mucho más preciso que repartir el consumo global en partes iguales.
    const ultimoFecha = fechasFiltradas[fechasFiltradas.length - 1];
    const ultimo = partes[ultimoFecha];
    const normSvc = name => {
      const u = String(name || '').toUpperCase();
      if (u.includes('SEAV')) return 'SEAV';
      if (u.includes('SBGC')) return 'SBGC';
      if (u.includes('VISA')) return 'VISA FLOTA';
      if (u.includes('VALES') || u.includes('ACA')) return 'VALES ACA';
      return u;
    };
    const svcList = ['SEAV', 'SBGC', 'VISA FLOTA', 'VALES ACA'];
    const consumoSvc = {};
    svcList.forEach(svc => {
      const deltas = [];
      for (let i = 1; i < fechasFiltradas.length; i++) {
        const getSaldo = f => {
          const r = (partes[f]?.datos_completos?.saldosServicios || []).find(s => normSvc(s.servicio) === svc);
          return r ? (parseFloat(r.saldo_total) || 0) : null;
        };
        const a = getSaldo(fechasFiltradas[i - 1]);
        const b = getSaldo(fechasFiltradas[i]);
        if (a !== null && b !== null) {
          const d = a - b; // baja de saldo = consumo del día
          if (d > 0) deltas.push(d);
        }
      }
      if (deltas.length) {
        deltas.sort((x, y) => x - y);
        consumoSvc[svc] = deltas[Math.floor(deltas.length / 2)]; // mediana
      }
    });
    // Respaldo si un servicio no tiene deltas válidos: consumo global promedio / 30 días
    const fallbackDiario = avg(resumenes, 'consumo_acumulado') / 30;
    const DIAS_PERIODO = { mensual: 30, cuatrimestral: 120, semestral: 180, anual: 360 };
    const diasPeriodo = DIAS_PERIODO[periodo] || 30;

    const tbody = document.getElementById('prev-tabla-body');
    if (tbody) {
      tbody.innerHTML = svcList.map(svc => {
        const saldoSvc = (ultimo?.datos_completos?.saldosServicios || []).find(s => normSvc(s.servicio) === svc);
        const saldo = saldoSvc ? (parseFloat(saldoSvc.saldo_total) || 0) : 0;
        const consumoDiario = consumoSvc[svc] || fallbackDiario;
        const fuente = consumoSvc[svc] ? 'histórico' : 'estimado';
        const proy = consumoDiario * diasPeriodo;
        const saldoProy = saldo - proy;
        const estado = saldoProy > 0 ? 'ok' : saldoProy > -proy ? 'adv' : 'crit';
        const estadoLabel = estado === 'ok' ? 'OK' : estado === 'adv' ? 'Atención' : 'Crítico';
        return `<tr><td>${svc}</td><td style="text-align:center;">${fmtMoney(saldo)}</td><td style="text-align:center;">${fmtMoney(consumoDiario)} <span style="color:var(--text-muted);font-size:10px;">(${fuente})</span></td><td style="text-align:center;">${fmtMoney(proy)}</td><td style="text-align:center;">${fmtMoney(saldoProy)}</td><td style="text-align:center;"><span class="estado-badge estado-${estado}">${estadoLabel}</span></td></tr>`;
      }).join('');
    }
    ...
    if (alertas) {
      ...
      alertas.innerHTML = `
        <div class="prev-alerta info"><div class="prev-alerta-icon">📊</div><div class="prev-alerta-content"><h4>Metodología de proyección</h4><p>Consumo diario = <strong>mediana de las bajas de saldo</strong> entre partes consecutivos del mismo servicio (ignora acreditaciones y OP). Servicios sin historial suficiente usan el promedio global como respaldo. Período proyectado: <strong>${diasPeriodo} días</strong>.</p></div></div>
...
function classifyAndParse(filename, wb) {
  ...
  if (lowerName.includes('planilla') || lowerName.includes('diaria') || sheets.includes('SALDOS SERVICIOS')) {
    tipo = 'planilla';
  } else if (lowerName.includes('gc') || lowerName.includes('guardacosta') || sheets.includes('SALDOS GC') || sheets.includes('Condicion GC')) {
    tipo = 'gc';
  ...
  parsePlanillaDiaria(wb);
  parseSaldosGC(wb);
  parseConsumoDiario(wb);
}

function parsePlanillaDiaria(wb) {
  // Conversión numérica segura: '#DIV/0!', texto y vacíos -> 0 (no rompe la carga)
  const toNum = v => {
    ...
    for (let i = 0; i < json.length; i++) {
      ...
      if (row[0] && ['SEAV', 'SBGC', 'VISA FLOTA', 'VALES ACA'].some(s => String(row[0]).includes(s))) {
        STATE.data.saldosServicios.push({
          ...
          saldo_informado: toNum(row[1]),
          op_pendiente: row[2] || '',
          monto_op: toNum(row[3]),
          ff: row[4] || '',
          fecha_dispo: fmtDate(row[5]),
          saldo_total: toNum(row[6])
        });
      }
    }
  }
  ...
  if (wb.SheetNames.includes('SALDOS DTRA')) {
    ...
    STATE.data.ypfTotalesDTRA = {};
    const clavesYPF = {
      'CREDITO INICIAL JUNIO': 'credito_inicial',
      'INCREMENTO/DISMINUCION': 'incremento',
      'SALDO TERRESTRE ACTUAL': 'saldo_terrestre',
      'SALDO SUPERFICIE ACTUAL': 'saldo_superficie',
      'SALDO TOTAL': 'saldo_total',
      'CONSUMO TOTAL GRAL': 'consumo_total'
    };
    for (let i = 0; i < json.length; i++) {
      const row = json[i];
      if (!row[0]) {
        // Bloque YPF: etiqueta en col B, valor en col C
        const clave = clavesYPF[String(row[1] || '').trim()];
        if (clave) STATE.data.ypfTotalesDTRA[clave] = toNum(row[2]);
        continue;
      }
      if (String(row[0]).includes('SERVICIO') || String(row[0]).includes('VISA') || String(row[0]).includes('VALES')) {
        STATE.data.saldosDTRA.push({
          concepto: String(row[0]).trim(),
          saldo_actual: toNum(row[1]),
          consumo: toNum(row[3]),
          prevision: toNum(row[5])
        });
      }
    }
  }
  ...
  if (wb.SheetNames.includes('YPF EN RUTA')) {
    ...
    for (let i = 0; i < json.length; i++) {
      const row = json[i];
      // Buscar "CONCEPTO: | SUPERFICIE" en CUALQUIER columna de la fila
      // (en la planilla está en K/L, no en col C: por eso antes no cambiaba de modo)
      for (let c = 0; c < row.length - 1; c++) {
        if (row[c] && String(row[c]).toUpperCase().includes('CONCEPTO') &&
          row[c + 1] && String(row[c + 1]).toUpperCase().includes('SUPERFICIE')) {
          mode = 'superficie';
        }
      }
      const first = String(row[0] ?? '').trim();
      // Solo filas de contrato: numero puro en col A.
      // Salta encabezados, filas de totales (col A vacia) y filas decorativas (MES/DIA).
      if (!/^\d+$/.test(first)) continue;
      const obj = {
        n_contrato: parseInt(first, 10),
        contrato: String(row[1] || '').trim(),
        asignado_inicio: toNum(row[2]),
        asignado_mes: toNum(row[3]),
        consumo_actual: toNum(row[4]),
        consumo_prom_dia: toNum(row[5]),
        saldo_actual: toNum(row[6]),
        dias_restantes: toNum(row[7]),
        consumo_dias_rest: toNum(row[8]),
        diferencia: toNum(row[9])
      };
...
function parseSaldosGC(wb) {
  // Parsea litros que vienen como numero o como texto tipo "12.626 G.O HF"
  const parseLts = v => {
    if (typeof v === 'number') return v;
    ...
    else if (s.includes('.')) s = s.replace(/\./g, ''); // punto = separador de miles
  ...
  // La planilla actual usa la hoja 'Condicion GC' (la vieja era 'SALDOS GC')
  const saldosSheet = wb.SheetNames.includes('SALDOS GC') ? 'SALDOS GC'
    : (wb.SheetNames.includes('Condicion GC') ? 'Condicion GC' : null);

  if (saldosSheet) {
    const ws = wb.Sheets[saldosSheet];
    const json = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
    STATE.data.gcSaldos = [];
    for (let i = 0; i < json.length; i++) {
      const row = json[i];
      if (row[0] && (String(row[0]).startsWith('GC-') || String(row[0]).startsWith('SB-'))) {
        STATE.data.gcSaldos.push({
          ...
          saldo_litros: parseLts(row[1]),
          porcentaje: toNum(row[4]),
          capacidad: toNum(row[8]),
          consumo_dia: toNum(row[9])
        });
      } else if (STATE.data.gcSaldos.length && String(row[0] ?? '').trim() === '' && row[1] &&
        /[\d.,]+\s+\w/.test(String(row[1]))) {
        // Filas de combustible adicional del GC anterior (ej.: GC-187 tiene
        // G.O HF + EURO D. + INFINIA D. + CABOTAJE en filas sueltas)
        STATE.data.gcSaldos[STATE.data.gcSaldos.length - 1].saldo_litros += parseLts(row[1]);
      }
    }
  }
  ...
  if (wb.SheetNames.includes('Condicion GC')) {
    ...
    for (let i = 0; i < json.length; i++) {
      ...
      } else if (STATE.data.gcCondicion.length && String(row[0] ?? '').trim() === '' && row[1] &&
        /[\d.,]+\s+\w/.test(String(row[1]))) {
        // Combustible adicional del GC anterior (filas sueltas tipo "8.000 INFINIA D.")
        STATE.data.gcCondicion[STATE.data.gcCondicion.length - 1].saldo_litros += parseLts(row[1]);
      }
    }
  }
...
function parseConsumoDiario(wb) {
  const sheetName = wb.SheetNames.find(s => s.includes('POR DIA'));
  if (!sheetName) return;
...
function updateKPIs() {
  ...
  document.getElementById('kpi-gc-total').textContent = gcCount;
...
function switchTab(tabName) {
  ...
  if (tabName === 'admin') {
    if (typeof cargarUsuariosDesdeNube === 'function') {
      cargarUsuariosDesdeNube().then(() => {
        if (typeof renderTablaUsuarios === 'function') renderTablaUsuarios();
      });
    }
  }
  if (tabName === 'repositorio') {
    if (typeof listarPartesGuardados === 'function') listarPartesGuardados();
  }
}
...
function renderDataTable(tableId, data, columns, paginate = true) {
  ...
  if (paginate) {
    ...
    tbody.innerHTML = filtered.map(row => `<tr>${columns.map(col => {
      ...
      return `<td class="${cls}" style="${col.align ? 'text-align:' + col.align + ';' : 'text-align:center;'}">${content}</td>`;
    }).join('')}</tr>`).join('') || '<tr><td colspan="' + columns.length + '" style="text-align:center;padding:40px;color:#5a6a82;">Sin datos</td></tr>';
  }
}
...
function renderAllCharts() {
  ...
  if (STATE.data.saldosServicios.length) {
    ...
    if (ctx) {
      // Función para extraer nombre corto del servicio
      function cleanServiceLabel(name) {
        if (!name) return 'OTROS';
        const upper = String(name).toUpperCase();
        if (upper.includes('SEAV')) return 'SEAV';
        if (upper.includes('SBGC')) return 'SBGC';
        if (upper.includes('VISA FLOTA') || upper.includes('VISA')) return 'VISA FLOTA';
        if (upper.includes('VALES ACA') || upper.includes('ACA')) return 'VALES ACA';
        return 'OTROS';
      }
      // Colores fijos por servicio
      const colorMap = {
        'SEAV': '#00d4ff',
        'SBGC': '#00e676',
        'VISA FLOTA': '#ffab00',
      ...
      // Agrupar y sumar saldos por servicio limpio
      const grouped = {};
      STATE.data.saldosServicios.forEach(s => {
        const label = cleanServiceLabel(s.servicio);
        grouped[label] = (grouped[label] || 0) + (parseFloat(s.saldo_total) || 0);
      });
      // Orden fijo: SEAV, SBGC, VISA FLOTA, VALES ACA, OTROS
      const order = ['SEAV', 'SBGC', 'VISA FLOTA', 'VALES ACA', 'OTROS'];
      const labels = [];
      const data = [];
      const bgColors = [];
      order.forEach(key => {
            ...
            tooltip: {
              ...
              callbacks: {
                label: function (ctx) {
                  ...
                  const total = parseFloat(grandTotal) || 1;
